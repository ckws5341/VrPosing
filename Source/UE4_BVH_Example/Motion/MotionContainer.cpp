...
#elif defined WIN32
// include <windows.h>
// https://answers.unrealengine.com/questions/811727/view.html
...
const Motion* 
MotionContainer::getMotion(int i) const
{
...
const Motion*
MotionContainer::getMotion(std::string name) const
{
	MotionPList::const_iterator iter;
	for ( iter=motions_.cbegin(); iter!=motions_.cend(); iter++ )
	{
...
void MotionContainer::addMotionBvhSetDir(std::string dir_path, std::string prefix_of_file, double scale, unsigned int max_byte, int num)
{
	...
	fm.FindFiles(find_file_data, *FString(reg_exp.c_str()), true, false);

	for ( auto &d : find_file_data )
	{
		FString file = FString(dir_path.c_str()) + d;

		if ( max_byte == 0 || fm.FileSize(*file) <  max_byte )
		{
			addMotionBvhFile(std::string(TCHAR_TO_UTF8(*file)), "", scale);
			count++;
		}
		if ( num == count ) break;
	}
...
#ifdef UE4
	amc_reader.LoadAMC_UE4(motion_file.c_str(), asf_file.c_str(), motion, true, 2.54);
#else
...
#ifdef UE4
	UE_LOG(LogTemp, Warning, TEXT("%s, %s"), *(FString(name.c_str())), *(FString(motion_file.c_str())));
#endif
...
MotionContainer::addMotionDir(std::string asf_file, std::string dir_path, std::string prefix_of_file, unsigned int max_byte, int max_file_num)
{
	...
	fm.FindFiles(find_file_data, *FString(reg_exp.c_str()), true, false);
	UE_LOG(LogTemp, Warning, TEXT("files %d"), find_file_data.Num());
	...
	for ( auto &d : find_file_data )
	{
		...
		if ( max_byte == 0 || fm.FileSize(*file) <  max_byte )
		{
			addMotionFile(asf_file, std::string(TCHAR_TO_UTF8(*file)));
			count++;
		}
...
//////////////////////////////////////////////////////////////////////
// Algorithms
void SelectKeyFramesByKineticEnergy(const MotionContainer *mc , MotionKeyFrames &out_key_frames, int smooth_times)
{
	for ( Motion* m : *mc )
	{
		std::vector<int> key_frames;
		SelectKeyFramesByKineticEnergy(m, key_frames, smooth_times);
