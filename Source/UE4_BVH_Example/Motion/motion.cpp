...
namespace ml
{
	...
	void Motion::Stitch_UE4(const Motion & const_add_m, bool forward)
	{
		...
		cml::matrix3 diff_rot = PlaneProject_mat3(ori_p.rotate(0) * cml::inverse(add_p.rotate(0)));
		double diff_time = ori_p.time - add_p.time;
		...
		for (int i = start_frame; i <= end_frame; i++)
		{
			...
			for (int j = 0; j < (int)body->num_joint(); j++)
			{
					...
					)
				{
					...
					if (body->joint_tag(j) == JointTag::PELVIS)
					{
	...
	void SelectKeyFramesByKineticEnergy(const Motion *m, std::vector<int> &out_key_frames, int smooth_times)
	{
		//m->
