...
namespace ml
{
	...
	void Posture::IkLimb(int end_joint, int lower_joint, int upper_joint, const cml::vector3& pos, bool reverse)
	{
		...
		double a = cml::acos_safe((l*l + n * n - m * m) / (2 * l*n));
		double b = cml::acos_safe((l*l + m * m - n * n) / (2 * l*m));
		...
		double a_ = cml::acos_safe((l_*l_ + n * n - m * m) / (2 * l_*n));
		double b_ = cml::acos_safe((l_*l_ + m * m - n * n) / (2 * l_*m));
		...
		double rot2 = cml::acos_safe((l_new * l_new + l * l - l_diff * l_diff) / (2.0 * l_new * l));
