...
namespace ml
{
	...
	void
	BVHReader::LoadBVH_UE4(const char *file, Motion *motion, bool root_offset, bool human_load, double scale, int sample)
	{
		...
		while (in) {
			...
			if (bufstr == "OFFSET") {
				...
				y *= scale;
				z *= scale;
				m_joints[cur_node].offset = convert_unreal_m * cml::vector3(x, y, z);
				continue;
			}
			if (bufstr == "CHANNELS") {
				int n;
	...
	void AMCReader::LoadAMC_UE4(const char *amc_file, const char *asf_file, Motion *motion, bool human_load, double scale, int sample)
	{
		cml::matrix3 convert_unreal_m(
			1, 0, 0, 
			0, 0, 1, 
		...
		while (in)
		{
			...
			m_joints[cur_node].dir = convert_unreal_m *  vector3(tx, ty, tz) * s * scale;
			m_joints[cur_node].m = convert_unreal_m *  rotz_mat(rz) * roty_mat(ry) * rotx_mat(rx) * convert_unreal_m_inv;
			m_joints[cur_node].is_parent = false;
		}
		// read 'begin'
		in >> bufstr;
		...
		while (in) {
			...
			for (int i = 0; i < (int)joints.size() - num_leaf; ++i) {
				...
				if (token == "root") {
					...
					ty *= scale;
					tz *= scale;
					p.trans(convert_unreal_m *vector3(tx, ty, tz));
					rx = M_PI * rx / 180;
					ry = M_PI * ry / 180;
					rz = M_PI * rz / 180;
